...
SUCTION_BASE_URDF = 'assets/ur5/suction/suction-base.urdf'
SUCTION_HEAD_URDF = 'assets/ur5/suction/suction-head.urdf'

class Gripper:
    """Base gripper class."""

    def __init__(self, assets_root, client):
...
class Suction(Gripper):
    ...
    def __init__(self, assets_root, robot, ee, obj_ids, client):
        ...
        """
        super().__init__(assets_root, client)

        # Load suction gripper base model (visual only).
        pose = ((0.487-1., 0.109+0.6, 0.438+1.07236), self.client.getQuaternionFromEuler((np.pi, 0, 0)))
        # base = pybullet_utils.load_urdf(
        #     p, os.path.join(self.assets_root, SUCTION_BASE_URDF), pose[0], pose[1])
        base = self.client.loadURDF(
            fileName=os.path.join(self.assets_root, SUCTION_BASE_URDF),
            basePosition=pose[0],
        ...
        pose = ((0.487-1, 0.109+0.6, 0.347+1.07236), self.client.getQuaternionFromEuler((np.pi, 0, 0)))
        # self.body = pybullet_utils.load_urdf(
        #     p, os.path.join(self.assets_root, SUCTION_HEAD_URDF), pose[0], pose[1])
        self.body = self.client.loadURDF(
            fileName=os.path.join(self.assets_root, SUCTION_HEAD_URDF),
            basePosition=pose[0],
            baseOrientation=pose[1]
        )
        constraint_id = self.client.createConstraint(
            parentBodyUniqueId=robot,
            parentLinkIndex=ee,
        ...
        self.client.changeConstraint(constraint_id, maxForce=50)
    ...
    def activate(self):
        ...
        if not self.activated:
            points = self.client.getContactPoints(bodyA=self.body, linkIndexA=0)
        # print(points)
            if points:
                ...
                if obj_id in self.obj_ids['rigid']:
                    body_pose = self.client.getLinkState(self.body, 0)
                    obj_pose = self.client.getBasePositionAndOrientation(obj_id)
                    world_to_body = self.client.invertTransform(body_pose[0], body_pose[1])
                    obj_to_body = self.client.multiplyTransforms(world_to_body[0],
                                                        world_to_body[1],
                                                        obj_pose[0], obj_pose[1])
                    self.contact_constraint = self.client.createConstraint(
    ...
    def release(self):
        ...
        # Release gripped rigid object (if any).
            if self.contact_constraint is not None:
                try:
                    self.client.removeConstraint(self.contact_constraint)
                    self.contact_constraint = None
            ...
            if self.def_grip_anchors:
                for anchor_id in self.def_grip_anchors:
                    self.client.removeConstraint(anchor_id)
                self.def_grip_anchors = []
                self.def_grip_item = None
                self.def_min_vetex = None
                self.def_min_distance = None
    ...
    def detect_contact(self):
        ...
        if self.activated and self.contact_constraint is not None:
            try:
                info = self.client.getConstraintInfo(self.contact_constraint)
        ...
        points = self.client.getContactPoints(bodyA=body, linkIndexA=link)
        # print(points)
        # exit()
        if self.activated:
    ...
    def check_grasp(self):
        ...
        if self.contact_constraint is not None:
            suctioned_object = self.client.getConstraintInfo(self.contact_constraint)[2]
